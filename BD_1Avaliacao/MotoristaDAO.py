...
class MotoristaDAO:
    ...
    def create_motorista(self, motorista: list, nota: int):
        try:
            res = self.db.collection.insert_one({"motoristas": motorista, "nota": nota})
    ...
    def create_corrida(self, corrida: list, id_motorista: int):
        try:
            res = self.db.collection.insert_one({"corridas": corrida, "id": id_motorista})
            print(f"Person created with id: {res.inserted_id}")
            return res.inserted_id
        except Exception as e:
            print(f"An error occurred while creating person: {e}")
    ...
    def update_person(self, id: str, motorista: list, nota: int):
        try:
