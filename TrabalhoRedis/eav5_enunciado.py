...
def questao_2(interests):
    ...
    for user_data in interests:
        ...
        # Armazenando os interesses no Redis
        for interesse in interesses:
            for nome_interesse, pontuacao in interesse.items():
                redis_conn.zadd(f"usuario:{user_id}:interesses", {nome_interesse: pontuacao})

        # Recuperando os interesses do Redis
        interesses_salvos = redis_conn.zrange(f"usuario:{user_id}:interesses", 0, -1, withscores=True)

        # Formatando o resultado como desejado
        formatted_interests = {
            'usuario': user_id,
            'interesses': interesses_salvos
        }
...
# Questão 4
def questao_4(post_id):
    user_interests = redis_conn.zrange(f"usuario:{post_id}:interesses", 0, -1, withscores=True)
    interests = {interest: score for interest, score in user_interests}
    post_ids = redis_conn.keys("post:*")
    posts = []

    for post_id in post_ids:
        post_data = redis_conn.hgetall(post_id)
        post_tags = redis_conn.smembers(f"{post_id}:tags")
        posts.append({
            'id': post_id,
    ...
    # Filtrar e ordenar os posts baseados nos interesses do usuário
    recommended_posts = []

    for post in posts:
        relevance_score = sum(interests.get(tag, 0) for tag in post['tags'])
        if relevance_score > 0:
            recommended_posts.append((post, relevance_score))

    # Ordenar os posts pelo score de relevância em ordem decrescente
    recommended_posts.sort(key=lambda x: x[1], reverse=True)

    return [post for post, score in recommended_posts]
...
def test_questao_4():
    ...
    assert output == questao_4(post_id)
